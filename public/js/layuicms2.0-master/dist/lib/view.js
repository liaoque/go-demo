...
;layui.define(["laytpl", "layer"], function (e) {
    ...
    i.loading = function (e) {
        ...
        return delete e.success, delete e.error, t.ajax(t.extend({
            type: e.type|"get", dataType: "json", success: function (t) {
                var n = s.statusCode;
                ...
                if(e.responseText.length){
                    var parse = JSON.parse(e.responseText);
                    var n = s.statusCode;
                    if (parse[s.statusName] == n.logout) i.exit();
                    mes = parse.mes;
                    layui.$("body").triggerHandler("click")
                }
                var a = ["请求异常，请重试<br><cite>错误信息：</cite>" + mes, d()].join("");
                $('#submit-btn').text($('#submit-btn').data('text')).removeAttr("disabled").removeClass("layui-disabled");
